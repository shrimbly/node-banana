...
function createCredentialStore(directory, safeStorage, onSecrets = () => {}) {
  ...
  function read() {
    ...
    try {
      ...
      return value;
    } catch { throw new Error('Stored keys could not be decrypted. Unlock your login keychain and retry. Existing credentials have been preserved.'); }
  }
