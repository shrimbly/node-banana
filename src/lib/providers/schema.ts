 ...
 * Model Schemas
 *
 * The parameters (settings) and inputs (connectable handles) of one model,
 * as GET /api/models/:modelId serves them. Kept here so other server code
 * (the in-app agent) can read a model's schema with the route's caching and
 * extraction rules.
 *
 * Replicate and fal.ai schemas come from their OpenAPI documents; WaveSpeed
 * from its models listing (cached by ./registry) with a static fallback;
 * Kie.ai, OpenAI, Gemini and Comfy Router schemas are static.
 *
 * Server only.
 */
...
async function fetchReplicateSchema(
  ...
  // The id goes into the URL path unencoded; getModelSchema refuses bad ids
  // first, this keeps the fetcher safe on its own.
...
/** Providers that serve model schemas. */
export const SCHEMA_PROVIDERS = ["replicate", "fal", "kie", "wavespeed", "gemini", "openai", "comfy"] as const;

export type SchemaProvider = (typeof SCHEMA_PROVIDERS)[number];

export function isSchemaProvider(provider: string | null | undefined): provider is SchemaProvider {
  return !!provider && (SCHEMA_PROVIDERS as readonly string[]).includes(provider);
}

export interface ModelSchemaSuccess {
  ok: true;
  parameters: ModelParameter[];
  inputs: ModelInput[];
  cached: boolean;
...
export interface ModelSchemaFailure {
  ok: false;
  error: string;
  /** The HTTP status GET /api/models/:modelId answers with. */
  status: number;
}

export type ModelSchemaResult = ModelSchemaSuccess | ModelSchemaFailure;

export interface ModelSchemaOptions {
  /** Aborts the provider request (Replicate, fal.ai, WaveSpeed). */
  signal?: AbortSignal;
}

/**
 * The parameters and inputs of one model, as GET /api/models/:modelId
 * serves them. `modelId` is the plain id (already URL-decoded).
 *
 * Failures: 400 for an unknown provider or a malformed Replicate id, 401 for
 * Replicate or fal.ai without a key, 404 for an id outside the Comfy Router
 * catalog, 500 when the provider request throws. An unknown Gemini, Kie.ai,
 * OpenAI or fal.ai id is not an error: it yields empty lists, as the route
 * always has. Results are cached for ten minutes per provider and id.
 */
export async function getModelSchema(
  provider: ProviderType | string | null | undefined,
  modelId: string,
  keys: ProviderKeys,
  options: ModelSchemaOptions = {}
): Promise<ModelSchemaResult> {
  if (!isSchemaProvider(provider)) {
    return {
      ok: false,
  ...
  if (provider === "replicate" && !isValidReplicateModelId(modelId)) {
    return {
      ok: false,
      error: `Invalid Replicate model id "${modelId}". Expected "owner/name".`,
      status: 400,
    };
  }
  ...
  try {
    ...
    if (provider === "gemini") {
      ...
      const geminiVideoSchema = getGeminiVideoSchema(modelId);
      const geminiImageSchema = getGeminiImageSchema(modelId);
      ...
      result = await fetchReplicateSchema(modelId, apiKey, options.signal);
    } else if (provider === "kie") {
      // Kie.ai uses hardcoded schemas (no schema discovery API)
      result = getKieSchema(modelId);
    } else if (provider === "wavespeed") {
      // WaveSpeed uses dynamic schemas from API, with static fallback
      result = await fetchWaveSpeedSchema(modelId, keys.wavespeed || null, options.signal);
      ...
      result = await fetchFalSchema(modelId, apiKey, options.signal);
    }
